#!/bin/bash
# Nebulith — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything Nebulith needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine Nebulith
...
# Shell overrides (NEBULITH_PORT / NEBULITH_HOST) take top priority, then .env
# values (APP_PORT / APP_BIND), then built-in defaults.
PORT="${NEBULITH_PORT:-${APP_PORT:-7860}}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
HOST="${NEBULITH_HOST:-${APP_BIND:-127.0.0.1}}" # Set APP_BIND=0.0.0.0 in .env for LAN/Tailscale access.
PROBE_HOST="$HOST"
...
echo "▶ Nebulith quick start for macOS"
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
  ...
  echo "    NEBULITH_PORT=7900 ./start-macos.sh"
  exit 1
fi
...
echo "▶ Preparing Nebulith…"
NEBULITH_SKIP_RUN_HINT=1 ./venv/bin/python setup.py

# 5. Launch. Bind to loopback by default; opt into LAN/Tailscale with
#    NEBULITH_HOST=0.0.0.0.
URL_HOST="$HOST"
...
# NEBULITH_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$NEBULITH_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
  (
    for _ in $(seq 1 90); do
      if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
        printf '\n'
        printf '  ┌────────────────────────────────────────────┐\n'
        printf '  │  ✓ Nebulith is ready — opening your browser  │\n'
        printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting Nebulith — it will open in your browser at $URL"
