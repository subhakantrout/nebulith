...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Nebulith' canonical OpenAI-style messages to native Ollama /api/chat.

    Nebulith carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/SubhakantaRout/nebulith")
        h.setdefault("X-OpenRouter-Title", "Nebulith")
    return h
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Nebulith-only metadata before sending messages to providers.
