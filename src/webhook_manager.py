...
def _utcnow() -> datetime:
    """Return naive UTC for existing DB columns while avoiding datetime.now(timezone.utc).replace(tzinfo=None)."""
...
class WebhookManager:
    ...
    async def deliver_test(self, webhook_id: str, url: str, encrypted_secret: Optional[str]):
        ...
        await self._deliver(webhook_id, url, decrypted, "webhook.test", {"message": "Test ping from Nebulith"})
    ...
    async def _deliver(self, webhook_id: str, url: str, secret: Optional[str], event: str, payload: dict):
        ...
        headers = {
            ...
            "X-Nebulith-Event": event,
            "User-Agent": "Nebulith-Webhook/1.0",
        }
        if secret:
            sig = hmac.new(secret.encode(), body.encode(), hashlib.sha256).hexdigest()
            headers["X-Nebulith-Signature"] = sig
