...
def test_serve_runner_preserves_command_exit_code():
    ...
    assert "NEBULITH_CMD_EXIT=$?" in script
    assert 'echo "=== Process exited with code $NEBULITH_CMD_EXIT ==="' in script
...
def test_llama_cpp_linux_bootstrap_checks_cudart_before_cuda_build():
    ...
    assert '_nebulith_has_cudart' in script
    ...
    assert script.index('_nebulith_has_cudart') < script.index('DGGML_CUDA=ON')
