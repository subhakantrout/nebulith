...
def _load_signature_cli(monkeypatch):
    ...
    path = Path(__file__).resolve().parent.parent / "scripts" / "nebulith-signature"
    loader = importlib.machinery.SourceFileLoader("nebulith_signature_cli_under_test", str(path))
