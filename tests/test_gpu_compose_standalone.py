...
base compose with only the matching overlay merged into the ``nebulith``
...
def _merge_overlay_into_base(base: dict, overlay: dict) -> dict:
    """Build the expected standalone config: base + overlay on nebulith only."""
...
# --- Non-nebulith services and volumes untouched ---------------------------


@pytest.mark.parametrize("standalone_path", [NVIDIA_STANDALONE, AMD_STANDALONE])
def test_non_nebulith_services_match_base(base, standalone_path):
...
# --- nebulith = base service + only the overlay additions ------------------


def test_nvidia_nebulith_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_nebulith_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
