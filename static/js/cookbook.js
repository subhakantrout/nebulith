  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false) {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside Nebulith.');
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section('Nebulith app', 'Run inside the Nebulith app itself.', _appDeps),
...
function _renderRecipes() {
  ...
  html += '<div class="hwfit-list-footer" style="margin-top:8px;padding-top:6px;border-top:1px solid color-mix(in srgb, var(--border) 50%, transparent);font-size:9.5px;opacity:0.65;text-align:right;">'
       + 'Don\'t see a model? '
       + '<a href="https://github.com/SubhakantaRout/nebulith/discussions/1962" target="_blank" rel="noopener" style="color:var(--accent,var(--red));text-decoration:none;display:inline-flex;align-items:center;gap:4px;vertical-align:middle;">'
       + 'Request it →'
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend Nebulith capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install Nebulith keys, choose model directories, and set the default server. Local is this machine.</p>';
