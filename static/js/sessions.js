...
let _sortMode = Storage.get('nebulith-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Nebulith Chat';
...
const FOLDER_STATE_KEY = 'nebulith-folder-state';
const FOLDER_ORDER_KEY = 'nebulith-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Nebulith kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Nebulith Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('nebulith-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('nebulith-session-sort', mode);
  else Storage.remove('nebulith-session-sort');
  renderSessionList();
}
