  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('nebulith-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('nebulith-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('nebulith-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('nebulith-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('nebulith-char-sessions', JSON.stringify(charSessions));
