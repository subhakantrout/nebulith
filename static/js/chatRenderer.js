...
const _ACTION_RECENTS_KEY = 'nebulith-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'nebulith-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Nebulith';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Nebulith' : shortModel(resolvedModel);
