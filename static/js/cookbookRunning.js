...
function _buildCrashReport(task, outputText) {
  ...
  const report = [
    '## Nebulith Cookbook crash report',
    '',
  ...
  if (task.payload) {
    if (task.type === 'serve' && task.payload._cmd) {
      ...
      _updateTask(task.sessionId, {
        status: 'running',
        output: `${task.output || ''}\n\n[nebulith] Retrying download. Progress may briefly look like a fresh download while HuggingFace checks cached/incomplete files; cached partial files will be reused when available.`.trim(),
        _retrying: true,
      });
