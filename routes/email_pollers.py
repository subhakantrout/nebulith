...
async def _auto_summarize_pass_single(days_back: int = 1, account_id: str | None = None, progress_cb=None) -> str:
    ...
    try:
        ...
        since = (datetime.now(timezone.utc).replace(tzinfo=None) - _td(days=max(1, days_back))).strftime("%d-%b-%Y")
                        ...
                        if resp.ok:
                            ...
                            if summary:
                                ...
                                _c.execute("""
                                    ...
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                                """, (message_id, uid.decode() if isinstance(uid, bytes) else str(uid), _folder, subject, sender, summary, model, datetime.now(timezone.utc).replace(tzinfo=None).isoformat()))
                                _c.commit()
                                _c.close()
                    ...
                    try:
                        ...
                        if reply:
                            ...
                            _c.execute("""
                                ...
                                VALUES (?, ?, ?, ?, ?, ?)
                            """, (message_id, uid.decode() if isinstance(uid, bytes) else str(uid), _folder, reply, model, datetime.now(timezone.utc).replace(tzinfo=None).isoformat()))
                            _c.commit()
                            _c.close()
                    ...
                    try:
                        ...
                        _cc.execute(
                            ...
                            (message_id, uid.decode() if isinstance(uid, bytes) else str(uid),
                             _cal_run_count, datetime.now(timezone.utc).replace(tzinfo=None).isoformat())
                        )
                        _cc.commit()
                        _cc.close()
                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Nebulith (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Nebulith" button
                                    import html as _h
                                    ...
                                    outer_alert["Date"] = datetime.now(timezone.utc).replace(tzinfo=None).strftime("%a, %d %b %Y %H:%M:%S +0000")
                        ...
                        if not resp.ok:
                            ...
                            if parsed is not None:
                                ...
                                """, (message_id, account_owner or "", uid.decode(), subject, sender,
                                      ...
                                      spam_reason, moved_to, model, datetime.now(timezone.utc).replace(tzinfo=None).isoformat()))
                                _c.commit()
                                _c.close()
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "nebulith_kind" if "nebulith_kind" in cols else "'scheduled' AS nebulith_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                nebulith_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Nebulith-Origin"] = "nebulith-ui"
                outer["X-Nebulith-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", nebulith_kind or "scheduled")[:64]
                outer["X-Nebulith-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `nebulith-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `NEBULITH_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `nebulith-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("NEBULITH_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `NEBULITH_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (NEBULITH_INPROCESS_POLLERS=0); "
            "drive `nebulith-mail poll-scheduled` externally."
        )
        return
    import asyncio
