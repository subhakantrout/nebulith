...
_HF_TOKEN_STATUS_SNIPPET = (
    'if [ -n "$HF_TOKEN" ]; then '
    'echo "[nebulith] HF token: applied"; '
    'else '
    'echo "[nebulith] HF token: NOT SET — gated/private models will be denied. '
    'Add one in Nebulith Settings -> Cookbook -> HuggingFace Token."; '
    'fi'
)
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def generate_cookbook_ssh_key(request: Request):
        ...
        if not key_path.exists():
            ...
            proc = await asyncio.create_subprocess_exec(
                ssh_keygen, "-t", "ed25519", "-N", "", "-C", "nebulith-cookbook", "-f", str(key_path),
    ...
    def _user_shell_path_bootstrap() -> list[str]:
        return [
            'NEBULITH_USER_SHELL="${SHELL:-}"',
            'if [ -n "$NEBULITH_USER_SHELL" ] && [ -x "$NEBULITH_USER_SHELL" ]; then',
            '  NEBULITH_USER_PATH="$("$NEBULITH_USER_SHELL" -ic \'printf "__NEBULITH_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__NEBULITH_PATH__//p\' | tail -n 1 || true)"',
            '  if [ -n "$NEBULITH_USER_PATH" ]; then export PATH="$NEBULITH_USER_PATH:$PATH"; fi',
            'fi',
        ]
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        # When Nebulith runs from a venv (e.g. native macOS install), put its bin
    ...
    async def model_cached(request: Request, host: str | None = None, model_dir: str | None = None, ssh_port: str | None = None, platform: str | None = None):
        ...
        if host:
            ...
            # LOCAL scan: use sys.executable (the venv Python Nebulith is already
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            runner_lines = ["#!/bin/bash"]
            runner_lines.extend(_user_shell_path_bootstrap())
            runner_lines.append('NEBULITH_PREFLIGHT_EXIT=""')
            # Put Nebulith's own venv bin on PATH (local runs only) so the serve
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                runner_lines.append('    NEBULITH_PREFLIGHT_EXIT=127')
                runner_lines.append('  fi')
                runner_lines.append('fi')
            elif "ollama" in req.cmd:
                ...
                runner_lines.append(f'NEBULITH_OLLAMA_HOST={_bash_squote(_ollama_host)}')
                runner_lines.append(f'NEBULITH_OLLAMA_PORT="{_ollama_port}"')
                runner_lines.append('NEBULITH_OLLAMA_URL=""')
                runner_lines.append('for _ody_ollama_try in $(seq 1 20); do')
                runner_lines.append('  for _ody_ollama_port in "$NEBULITH_OLLAMA_PORT" 11434; do')
                ...
                runner_lines.append('        NEBULITH_OLLAMA_URL="$_ody_ollama_url"')
                runner_lines.append('        NEBULITH_OLLAMA_PORT="$_ody_ollama_port"')
                runner_lines.append('        break 3')
                runner_lines.append('      fi')
                runner_lines.append('    done')
                runner_lines.append('  done')
                runner_lines.append('  [ "$_ody_ollama_try" -eq 1 ] && echo "[nebulith] Waiting for an existing Ollama API on ports ${NEBULITH_OLLAMA_PORT}/11434..."')
                runner_lines.append('  sleep 1')
                runner_lines.append('done')
                runner_lines.append('if [ -n "$NEBULITH_OLLAMA_URL" ]; then')
                runner_lines.append('  if [ "$NEBULITH_OLLAMA_PORT" != "' + _ollama_port + '" ]; then')
                runner_lines.append('    echo "[nebulith] Selected Ollama port ' + _ollama_port + ' was not reachable; using running Ollama on port ${NEBULITH_OLLAMA_PORT}."')
                runner_lines.append('  fi')
                runner_lines.append('  echo "[nebulith] Ollama API ready on port ${NEBULITH_OLLAMA_PORT}: ${NEBULITH_OLLAMA_URL}"')
                runner_lines.append('  echo "[nebulith] This task is monitoring an existing Ollama server; stopping it here will not stop an external Docker/system service."')
                runner_lines.append('  exec bash -i')
                runner_lines.append('fi')
                runner_lines.append('if ! command -v ollama &>/dev/null; then')
                runner_lines.append('  echo "ERROR: Ollama not found and no Ollama API is reachable on 127.0.0.1, localhost, or host.docker.internal (ports ${NEBULITH_OLLAMA_PORT}/11434)."')
                ...
                runner_lines.append('  exec bash -i')
                runner_lines.append('fi')
                runner_lines.append('NEBULITH_OLLAMA_URL="http://${NEBULITH_OLLAMA_HOST}:${NEBULITH_OLLAMA_PORT}"')
                if remote and _ollama_host in ("0.0.0.0", "::"):
                    runner_lines.append('echo "[nebulith] WARNING: remote Ollama will bind to ${NEBULITH_OLLAMA_HOST}:${NEBULITH_OLLAMA_PORT} so Nebulith can reach it from this host."')
                    runner_lines.append('echo "[nebulith] Ollama has no built-in authentication; expose this only on a trusted LAN/VPN or provide an explicit OLLAMA_HOST with your own access controls."')
                runner_lines.append('echo "Starting ollama server on ${NEBULITH_OLLAMA_HOST}:${NEBULITH_OLLAMA_PORT}..."')
                runner_lines.append('OLLAMA_HOST="${NEBULITH_OLLAMA_HOST}:${NEBULITH_OLLAMA_PORT}" ollama serve')
                ...
                runner_lines.append('  NEBULITH_PREFLIGHT_EXIT=1')
                runner_lines.append('fi')
